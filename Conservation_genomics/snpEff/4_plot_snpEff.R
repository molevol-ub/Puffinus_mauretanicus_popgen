...
pdf("snpEff.wPP.def.pdf", width=22, height=10)
...
pdf("snpEff.def.pdf", width=5, height=6.5)
manhplot <- ggplot(results_file, aes(x = POP, y=COUNT, fill=POP, alpha=GENOTYPE)) + 
	...
	labs(x="Population", y="# HIGH risk mutations") +
...
manhplot <- ggplot(results_file, aes(x = TYPE, y=COUNT, fill=TYPE)) + geom_bar(stat="summary") + theme_classic() + labs(x="Type of pairwise comparison", y="# shared HIGH risk mutations") + theme (axis.text=element_text(size=11.5), axis.title=element_text(size=17)) + theme(legend.position="none")+geom_jitter(aes(y=COUNT),size=0.8, alpha=0.7, shape=16, position=position_jitter(0.25))+ geom_errorbar(aes(ymin=AVG-SD, ymax=AVG+SD), width=.3) + scale_x_discrete(limits=c("INTRA-POP","INTRA-TAXA","INTER-TAXA")) + scale_fill_brewer(palette="Dark2")
