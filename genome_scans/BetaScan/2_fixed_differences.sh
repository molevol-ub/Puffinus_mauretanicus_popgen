...
workdir=/users-d3/jferrer/gizquierdo/TFM/genome_scans/BetaScan/outgroup
cd $workdir
...
vcftools --gzvcf ~/gizquierdo/TFG/chr_split/vcfs/auto/Puffinus_SNP.maxmiss80.filtered.wPP.merged.nomono.masked.vcf.gz --keep ./lists/$dataset.list --recode --out $dataset
...
# IMPORTANT! The output you will use for the next steps are only the derived fixed differences found in P.puffinus!
